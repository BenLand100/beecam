...
def load_data(count_con, metric_con, min_date='2024-04-15', interval=5*60):    
    ...
    df_metrics = pd.read_sql_query(f"""
        SELECT 
            ...
            --avg(ambient_lux)/1000 as 'Illumination (klux)', 
            ...
            avg(temperature_2)*9/5+32 as 'Temp-2 (F)', avg(humidity_2) as 'Humidity-2 (%)',
...
def plot_data(df_combined):
    ...
    for metric in metrics:
        if ~df_combined[metric].isnull().all():
            fig.add_trace(
                ...
                secondary_y=('lux' in metric) # or 'mbar' in metric) #or 'Bee' in metric),
            )
    ...
    fig.update_yaxes(title_text="Bees | mbar | °C | °F | lbs | %", secondary_y=False, range=[0,100], fixedrange=True)
    fig.update_yaxes(title_text="dB-klux", secondary_y=True, range=[0,50], fixedrange=True)
